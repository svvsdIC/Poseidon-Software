(function()
{
    ...
    class conductivity_sensor
    {
		...
		var interfaces = os.networkInterfaces();
			    ...
			    deps.logger.debug('conductivity_sensor plugin loaded!');
            ...
            this.listeners =
            {
                ...
                mcuStatus: new Listener( self.globalBus, 'mcu.status', false, function( data )
                {
                    ...
                    if( 'conductivity' in data )
                    {
                        ...
                        var message = data.conductivity;
                        // Re-emit the message on the cockpit messaging bus (talks to the browser)
			var util = require('util');
	                self.cockpitBus.emit( 'plugin.conductivity_sensor.value', message );
                    }
                })
            }
        }
        ...
        start()
        {
          ...
          console.log("conductivity_sensor started!!");
        }
    ...
    {
        return new conductivity_sensor(name, deps);
    };
}());
